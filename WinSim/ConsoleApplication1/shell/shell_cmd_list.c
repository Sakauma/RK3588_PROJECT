...
extern void enable_statis(int argc,char * argv[]);
extern void asm_cmd_input(int argc,char * argv[]);
extern void 	img_cmd_input(int argc, char* argv[]);
...
const ShellCommand shellCommandList[] = 
{
	...
	SHELL_CMD_ITEM(
                   SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_MAIN)|SHELL_CMD_DISABLE_RETURN,
                   sten, enable_statis, statis enable or disable)	,
				   
	SHELL_CMD_ITEM(
                   SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_MAIN)|SHELL_CMD_DISABLE_RETURN,
                   asmctrl, asm_cmd_input, input command for asm debug)	,
    SHELL_CMD_ITEM(
                   SHELL_CMD_PERMISSION(0) | SHELL_CMD_TYPE(SHELL_TYPE_CMD_MAIN) | SHELL_CMD_DISABLE_RETURN,
                   imgctrl, img_cmd_input, saveimage control)
			   
};


/**
 * @brief shell閸涙垝鎶ょ悰銊ャ亣鐏忥拷
 * 
 */
